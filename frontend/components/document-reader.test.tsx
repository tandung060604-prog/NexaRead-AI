...
function mockReaderApi(options: MockOptions = {}) {
  ...
  const fetchMock = vi.fn().mockImplementation((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    return Promise.resolve(apiResponse({ ...documentDetail, status: status.status }));
  });
...
describe("DocumentReader", () => {
  ...
  it("shows a safe processing failure state", async () => {
    ...
    expect(await screen.findByText("Không thể xử lý tài liệu. File gốc vẫn được giữ an toàn.")).toBeInTheDocument();
  ...
  it("shows the OCR required fallback", async () => {
    ...
    expect(await screen.findByText(/OCR chưa được hỗ trợ/)).toBeInTheDocument();
  });
  ...
  it("renders an empty TOC state", async () => {
    ...
    expect(await screen.findByText("No table of contents detected.")).toBeInTheDocument();
  });
  ...
  it("navigates from a TOC item to its stable block anchor", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Reader Guide, page 1" }));
  ...
  it("performs lexical search", async () => {
    ...
    const input = await screen.findByLabelText("Search document");
  ...
  it("opens reader tools for the mobile layout", async () => {
    ...
    expect(screen.getByRole("dialog", { name: "Reader tools" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Close reader tools" })).toBeInTheDocument();
  });
  ...
  it("mounts only a small virtual window for 5,000 blocks", async () => {
    ...
    await screen.findByLabelText("Virtualized document content");
  ...
  it("shows a progress save failure without hiding reader content", async () => {
    ...
    expect(await screen.findByText("Save failed", {}, { timeout: 2_500 })).toBeInTheDocument();
  });
});
